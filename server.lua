...
AddEventHandler('bibiModz:alertPolice', function(coords, policeJob)
    ...
    for _, xPlayer in pairs(xPlayers) do
        if xPlayer.job.name == policeJob then
            TriggerClientEvent('esx:showNotification', xPlayer.source, "Un vol a été signalé. Rendez-vous à la position indiquée.")
            TriggerClientEvent('bibiModz:createBlipForPolice', xPlayer.source, coords)
        end
    end
end)
